...
export const PERMISSIONS = [
  ...
  // Warehouse
  { name: 'WAREHOUSE_VIEW',   description: 'Can view warehouse stock, arrivals and dispatches' },
  { name: 'WAREHOUSE_CREATE', description: 'Can create warehouse arrivals and dispatches' },
  { name: 'WAREHOUSE_DELETE', description: 'Can delete warehouse arrivals and dispatches' },
...
const LEGACY_MIGRATION_MAP: Record<string, string[]> = {
  ...
  MANAGE_PRODUCTS:  ['PRODUCTS_VIEW', 'PRODUCTS_CREATE', 'PRODUCTS_UPDATE', 'PRODUCTS_DELETE',
                     'UNITS_VIEW', 'UNITS_CREATE', 'UNITS_UPDATE', 'UNITS_DELETE'],
  MANAGE_SUPPLIERS: ['SUPPLIERS_VIEW', 'SUPPLIERS_CREATE', 'SUPPLIERS_UPDATE', 'SUPPLIERS_DELETE'],
  MANAGE_CLIENTS:   ['CLIENTS_VIEW', 'CLIENTS_CREATE', 'CLIENTS_UPDATE', 'CLIENTS_DELETE'],
  MANAGE_ORDERS:    ['ORDERS_VIEW', 'ORDERS_CREATE', 'ORDERS_UPDATE', 'ORDERS_DELETE'],
  MANAGE_WAREHOUSE: ['WAREHOUSE_VIEW', 'WAREHOUSE_CREATE', 'WAREHOUSE_DELETE',
                     'LOANS_VIEW', 'LOANS_UPDATE'],
  VIEW_INCOME:      ['INCOME_VIEW'],
  VIEW_REPORTS:     ['INCOME_VIEW'],
  MANAGE_SETTINGS:  ['SETTINGS_VIEW', 'SETTINGS_CREATE', 'SETTINGS_UPDATE', 'SETTINGS_DELETE'],
};
...
async function main() {
  ...
  for (const perm of PERMISSIONS) {
    const found = await prisma.permission.findUnique({ where: { name: perm.name } });
